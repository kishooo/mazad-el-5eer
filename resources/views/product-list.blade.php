					...
					<div class="col-xs-12 col-sm-8 col-md-9">
						<div class="right-products">
                            ...
                            @foreach($bidders as $bidder)
														<div class="list-text">
															<h3>{{$bidder->title}}</h3>
															<p id="I{{$y}}" data-checkwinner="{{$bidder->id}}"></p>
                              <p id="F{{$y}}" data-product="{{$bidder->id}}"></p>
                              <span id="{{$y}}" data-product="{{$bidder->id}}" data-buy="{{$bidder->buyNow}}" data-countdown="{{date('m,d,y H:i:s',strtotime($bidder->deadline))}}"></span>
															...
															<h5><del>${{$bidder->startPrice}}</del><span id="P{{$y}}">${{$bidder->currentPrice}}</span></h5>
															<p>{{$bidder->desc}}.</p>
															<div class="list-btn">
                                <input data-price="{{$bidder->currentPrice}}" value="{{$bidder->currentPrice}}" class="input" min="1" type="number" id="B{{$y}}" step=10>
                                <div  class="submit-text">
                                  <a class="bid" id="BU{{$y}}" data-id="{{$y}}" data-product="{{$bidder->id}}"  data-price="{{$bidder->currentPrice}}">Bid</a>
                                </div>
                                @if(!is_null($bidder->buyNow)||!empty($bidder->buyNow))
                                <div  class="submit-text">
                                  <a id="BN{{$y}}" class="buyNow" data-product="{{$bidder->id}}">Buy Now</a>
                                </div>
                                @endif
																<a href="#">wishlist</a>
